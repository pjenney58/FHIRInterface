...
namespace PalisaidMeta.Model
{
    ...
    /// <summary>
    /// Collectors are modules that collect data from sources and passes it on for transformation
    /// into the Palisaid format. There must be a specific collector for each target host and the
    /// host can use any protocols for data transfer.
    ///
    /// The CollectorConfig is a data record that describes a host. Given the config, a collector
    /// will configure itself to do work.
    /// </summary>
    public class CollectorConfig : Entity
    {
        ...
        public decimal TimerDurationHours { get; set; }
        public decimal TimerDurationDays { get; set; }
        ...
        // The transport protocols may be different depending on the target system, for example,
        // Epic delivers data in FHIR but tackes v2.5 for input.
        //
        // Inbound and outbound transmission is from Palisaid's perspective
        //
        // {protocol}Out => Host reads from Palisaid {protocol}In => Palisaid reads from host
        //
        // Example: DataProtocolId = DataProtocol.HL7_Fhirv4; TransportProtocolIn =
        // TransportProtocol.Xml; NetworkProtocolIn = NetworkProtocol.HTTPS;
        //
...
#pragma warning disable CS1998 // Async method lacks 'await' operators and will run synchronously
        ...
        // A collector can manage multiple hosts just by changing the protocols and transport
        // specifics. Calling Reconfigure raises an event indicating that options have changed and
        // itselfe needs to update
